...
#define DMA_IP_SW_MAJOR_VERSION_C                2
...
static Dma_Ip_ReturnType Static_Dma_Ip_SetLogicChannelGlobalConfig(const uint32 LogicCh, const Dma_Ip_GlobalConfigType * const GlobalConfig, boolean ResetState)
{
    ...
    if(DMA_IP_HARDWARE_VERSION_2 == LocHwVers)
    {
        ...
        LocListHwv2[0U].Value = (GlobalConfig->Interrupt.EnErrorInt ? TRUE : FALSE); 
    #endif
        ...
        LocListHwv2[1U].Value = (GlobalConfig->Interrupt.EnErrorInt ? TRUE : FALSE); 
    #endif
        LocListHwv2[2U].Param = DMA_IP_CH_SET_EN_ERROR_INTERRUPT;
        LocListHwv2[2U].Value = (GlobalConfig->Interrupt.EnErrorInt ? TRUE : FALSE);
        if(TRUE != ResetState)
        {
        ...
        else
        {
            ...
            LocListHwv2[3U].Value = (GlobalConfig->Priority.EnPreemption ? TRUE : FALSE);
        }
        LocListHwv2[4U].Param = DMA_IP_CH_SET_EN_PREEMPTION_PRIORITY;
        LocListHwv2[4U].Value = (GlobalConfig->Priority.EnPreemption ? TRUE : FALSE);
        LocListHwv2[5U].Param = DMA_IP_CH_SET_DIS_PREEMPT_PRIORITY;
        LocListHwv2[5U].Value = (GlobalConfig->Priority.DisPreempt ? TRUE : FALSE);
        /* Enable Request is loaded last into Channel's Configuration */
        LocListHwv2[6U].Param = DMA_IP_CH_SET_EN_HARDWARE_REQ;
        LocListHwv2[6U].Value = (GlobalConfig->Request.EnRequest ? TRUE : FALSE);
...
Dma_Ip_ReturnType Dma_Ip_Init(const Dma_Ip_InitType * const DmaInit)
{
    ...
    if(DMA_IP_STATUS_SUCCESS == LocStatus)
    {
        ...
        for(LocCfgIdx = 0U; LocCfgIdx < DMA_IP_NOF_CFG_LOGIC_CHANNELS; LocCfgIdx++)
        {
            ...
            if(DMA_IP_HARDWARE_VERSION_2 == LocHwVers)
            {
                LocHwTcdChAddr = (uint32)(&DMA_IP_HWV2_TCD_CH_VALUE(LocHwInst));
                OsIf_Trusted_Call1param(Mcl_Dma_SetUserAccessAllowed, LocHwTcdChAddr);
            }
        #endif /* #if (STD_ON == DMA_IP_HWV2_IS_AVAILABLE) */
        #endif /* DMA_IP_TCD_REG_PROT_AVAILABLE */
        #else
            /* Avoid 2.2 misra violation rule */
            (void)DMA_IP_HWV2_TCD_CH_VALUE(LocHwInst);
        #endif /* DMA_IP_REG_PROT_AVAILABLE */
        #endif /* (STD_ON == DMA_IP_USER_MODE_SUPPORT_IS_AVAILABLE) */
        #endif /* MCAL_ENABLE_USER_MODE_SUPPORT */
